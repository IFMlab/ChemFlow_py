...
templates = os.environ.get('CHEMFLOW_PY_HOME') + '/templates/config/smina/'
output = 'output.pdbqt'
scoring_functions = ("vina", 'vinardo', 'dkoes_scoring', 'dkoes_fast')
config_file_name = 'config.txt'
format_rec = 'mol2'
data = {'center': ['X', 'Y', 'Z'], 'size': [15, 15, 15], 'scoring': 'vinardo',
        'exhaustiveness': 8, 'energy_range': 3.0, 'n_poses': 10,
        'command': ['smina.static --config  ../config.txt']}


def check(data):
    if not shutil.which('smina.static'):
        error('Smina is not installed or on PATH, command "smina.static" not found')
        exit()
...
def parse(parser):
    ...
    parser.add_argument("-sf", "--scoring",
                        metavar='STRING',
                        default='vinardo', choices=scoring_functions,
...
def rescore(data, protocol):
    ...
    if not os.path.isfile('receptor.pdbqt'):
        subprocess.run(f'python2 {shutil.which("prepare_receptor4.py")} -r {"receptor.mol2"} -o {"receptor.pdbqt"}',
                       shell=True, check=True)  # overwrite by default
    with open(templates + 'rescore.txt', 'r') as f:
        raw_text = f.read()
    # inserting the parameter into the config file
    text = raw_text.format(center=data['center'], size=data['size'],
                           scoring=data['scoring'])
    # saving the config file
    filename = protocol + os.sep + 'config.txt'
    with open(filename, 'w') as file:
        file.write(text)
    data['command'] = ['smina.static --minimize --config  ../config.txt']
    return 'config.txt'
