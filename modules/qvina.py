...
data = {'center': ['X', 'Y', 'Z'], 'size': [15, 15, 15], 'scoring': 'vina',
        ...
        'command': 'qvina2.1 --config ../config.txt'}


def check(data):
    if not shutil.which('qvina2.1'):
        error('Qvina is not installed or on PATH, command "qvina2.1" not found')
        exit()
...
def rescore(data, protocol):
    dock(data, protocol)
    data['command']= 'qvina2.1  --score_only --config ../config.txt'
    return 'config.txt'
